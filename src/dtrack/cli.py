...
from dtrack.reporting import describe_snapshot, write_report
...
def inspect(
    ...
    snapshot = MigrationService().inspect(options)
    typer.echo(describe_snapshot(snapshot))
...
def schema(
    ...
    report = MigrationService().create_schema(options)
    typer.echo("Schema operation completed")
...
def migrate(
    ...
    snapshot, report = MigrationService().migrate(options)
    typer.echo(describe_snapshot(snapshot))
